...
class TaskMotorControl:
    ...
    def __init__(self, motor: RomiMotor, speed_data, encoder_data, act_speed):
        """ This class is the motor controller for a Romi DC motor.

            This class takes the latest speed given to it from the RomiControl task and sets the motor speed to
            the provided speed by running closed loop control of that speed through its motor object

            Args:
                motor: the motor that this task controls
                speed_data: data that is sent from RomiControl that tells MotorControl what to set the motor speed to
            """
    ...
    def run(self):
        """The generator function that runs within this class to set the new motor speed
            """
        while True:
